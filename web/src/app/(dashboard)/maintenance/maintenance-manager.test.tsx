...
it("requires saving the completion form before marking work complete", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "저장" }));
  await waitFor(() => expect(fetchMock).toHaveBeenCalledWith("/api/proxy/maintenance/m1", expect.objectContaining({ body: JSON.stringify({ dueDate: "2026-09-04", status: "Completed" }) })));
});
