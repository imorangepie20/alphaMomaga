...
it("allows early completion on the new Seoul day without assuming a completion date", async () => {
  ...
  await waitFor(() => expect(fetchMock).toHaveBeenCalledWith("/api/proxy/inspections/i", expect.objectContaining({ body: JSON.stringify({ scheduledDate: "2026-09-10", priority: "Urgent", status: "Completed", completedAt: "2026-09-06" }) })));
});
...
it("sends the actual completion date after confirmation", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "저장" }));
  await waitFor(() => expect(fetchMock).toHaveBeenCalledWith("/api/proxy/inspections/i", expect.objectContaining({ method: "PUT", body: JSON.stringify({ scheduledDate: "2026-09-01", priority: "Urgent", status: "Completed", completedAt: "2026-09-04" }) })));
});
