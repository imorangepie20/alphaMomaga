...
export function ReceiptHistory({ receipts, charges }: { receipts: PaymentReceipt[]; charges: MonthlyCharge[] }) {
  ...
  return <div className="space-y-3"><h3 className="text-base font-semibold">수납 이력</h3>{error && <p role="alert" className="text-sm text-destructive">{error}</p>}{receipts.length === 0 ? <p className="text-sm text-muted-foreground">등록된 수납 영수증이 없습니다.</p> : receipts.map((receipt) => <div key={receipt.id} className="rounded-lg border p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><span>{receipt.receivedDate} · ₩{receipt.amountWon.toLocaleString("ko-KR")} · {receipt.method}</span><span className={receipt.voidedAt ? "text-destructive" : "text-emerald-700"}>{receipt.voidedAt ? `취소됨: ${receipt.voidReason}` : "정상"}</span></div><p className="mt-2 text-muted-foreground">배분: {receipt.allocations.map((allocation) => { const charge = charges.find((item) => item.id === allocation.chargeId); return `${charge ? `${charge.billingMonth} ${charge.dueDate}` : allocation.chargeId} ${allocation.amountWon.toLocaleString("ko-KR")}원`; }).join(", ")}</p>{!receipt.voidedAt && <div className="mt-3 flex gap-2"><Input aria-label={`${receipt.id} 취소 사유`} placeholder="취소 사유" value={reason[receipt.id] ?? ""} onChange={(event) => setReason({ ...reason, [receipt.id]: event.target.value })} /><Button size="sm" variant="outline" onClick={() => voidItem(receipt)} disabled={pending === receipt.id}>{pending === receipt.id ? "처리 중..." : "영수증 취소"}</Button></div>}</div>)}</div>;
}
