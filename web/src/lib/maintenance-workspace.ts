...
export async function getMaintenanceWorkspace(): Promise<{ items: Maintenance[]; properties: Property[] }> {
  const apiUrl = getApiUrl();
  if (!apiUrl) throw new Error("Maintenance API is not configured");
