...
export async function AdminAccountPage({ mode }: { mode: "users" | "roles" | "settings" }) {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle>계정·역할 변경 안내</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><p>계정 생성·차단·역할 할당은 Auth0 관리 콘솔에서 처리합니다. 현재 앱에는 전체 사용자 조회 및 계정 변경 API가 연결되어 있지 않습니다.</p><p>역할의 권한 구성은 서버 정책으로 관리됩니다. 역할을 변경한 뒤에는 다시 로그인하여 API 적용 역할을 확인해 주세요.</p><p className="text-muted-foreground">직원 수, 승인 대기 계정, 구독 결제 정보는 확인할 데이터가 없어 표시하지 않습니다.</p><nav className="flex flex-wrap gap-4"><Link href="/auth/logout" className="underline">로그아웃</Link><Link href="/settings" className="underline">계정 설정 확인</Link>{mode !== "roles" && access.role.permissions.includes("user:manage") && <Link href="/admin/roles" className="underline">전체 역할 비교</Link>}</nav></CardContent></Card>
  </div>;
}
