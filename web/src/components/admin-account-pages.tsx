...
export async function AdminAccountPage({ mode }: { mode: "users" | "roles" | "settings" }) {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle>계정·역할 변경 안내</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><p>관리자는 사용자 관리 화면에서 계정 조회·초대 링크 발급·차단·운영 역할 변경을 처리할 수 있습니다. 기존 관리자 계정 변경과 관리자 승격은 Auth0 관리 콘솔에서 처리합니다.</p><p>역할의 권한 구성은 서버 정책으로 관리됩니다. 역할을 변경한 뒤에는 다시 로그인하여 API 적용 역할을 확인해 주세요.</p><p className="text-muted-foreground">직원 수, 승인 대기 계정, 구독 결제 정보는 확인할 데이터가 없어 표시하지 않습니다.</p><nav className="flex flex-wrap gap-4"><a href="/auth/logout" className="underline">로그아웃</a><Link href="/settings" className="underline">계정 설정 확인</Link>{access.role.permissions.includes("user:manage") && <Link href="/admin/users" className="underline">사용자 관리</Link>}{mode !== "roles" && access.role.permissions.includes("user:manage") && <Link href="/admin/roles" className="underline">전체 역할 비교</Link>}</nav></CardContent></Card>
  </div>;
}
