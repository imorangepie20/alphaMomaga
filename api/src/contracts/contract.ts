...
export function validateContract(contract: Contract, referenceDate = new Date()): void {
  ...
  const startIsValid = !Number.isNaN(startDate.getTime()) && startDate.toISOString().slice(0, 10) === contract.startDate;
  const endIsValid = !Number.isNaN(endDate.getTime()) && endDate.toISOString().slice(0, 10) === contract.endDate;
  if (!startIsValid || !endIsValid || contract.startDate >= contract.endDate) {
    throw new Error(`Contract ${contract.id} must have a valid start date before its end date`);
  }
  ...
  if (contract.status === 'Upcoming' && today >= startDate) {
    throw new Error(`Contract ${contract.id} cannot be upcoming after its start date`);
  }
  if (contract.status === 'Active' && (today < startDate || today > endDate)) {
    throw new Error(`Contract ${contract.id} must be within its active lease dates`);
  }
  if (contract.status === 'Expired' && today <= endDate) {
    throw new Error(`Contract ${contract.id} cannot be expired before its end date`);
  }
  if (contract.status === 'Terminated') {
    ...
    if (Number.isNaN(terminatedAt.getTime()) || terminatedAt < startDate || terminatedAt > today) {
      throw new Error(`Contract ${contract.id} has an invalid termination date`);
    }
  }
}