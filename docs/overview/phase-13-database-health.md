...
- `MaintenanceService`와 `InspectionsService` 조회도 `DATABASE_URL` 설정 시 PostgreSQL을 사용하도록 전환했습니다.
- 점검의 nullable `completed_at`을 선택 필드로 유지하고 유지보수·점검 결과를 ID 순서로 반환합니다.

## 현재 범위

Properties, Tenant, Contract, Payment, Maintenance, Inspection 조회는 DB가 설정된 경우 PostgreSQL을 사용하고, 나머지 도메인 서비스는 아직 메모리 모델을 사용합니다. 따라서 이 단계는 연결 상태, 종료 lifecycle, 여섯 도메인의 조회/생성 전환을 담당합니다. 실제 연결 테스트는 PostgreSQL 서버가 준비된 후 수행합니다. `DATABASE_URL`이 없으면 seed가 명확한 오류로 중단되며 기본 데이터베이스에 임의로 연결하지 않습니다.
...
- Maintenance 매핑 테스트: 작업 요청 행과 날짜 변환
- Inspection 매핑 테스트: 점검 행과 nullable 완료일 처리
