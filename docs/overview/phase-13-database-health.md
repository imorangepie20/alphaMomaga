...
Properties 조회는 DB가 설정된 경우 PostgreSQL을 사용하고, 나머지 도메인 서비스는 아직 메모리 모델을 사용합니다. 따라서 이 단계는 연결 상태, 종료 lifecycle, 첫 번째 읽기 전환을 담당하며, 임차인 생성과 나머지 조회는 아직 PostgreSQL에 저장되지 않습니다. 실제 연결 테스트는 PostgreSQL 서버가 준비된 후 수행합니다. `DATABASE_URL`이 없으면 seed가 명확한 오류로 중단되며 기본 데이터베이스에 임의로 연결하지 않습니다.
