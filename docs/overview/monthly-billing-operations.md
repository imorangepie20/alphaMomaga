...
사유를 입력하고 `영수증 취소`를 누르면 기존 테마의 확인창이 열린다.
영수증 번호·금액·사유를 확인한 뒤 `취소 확정`을 눌러야 API 요청이 실행된다.
`돌아가기`는 원장을 변경하지 않는다. 처리 중에는 중복 실행과 확인창 닫기를 막고,
401은 재로그인 안내, 403은 권한 안내로 구분한다. 실패 시 사유와 확인 정보를 유지한다.

실제 로그인 브라우저 검수에서 서버 HTML 표시 직후 클릭이 반응하지 않는 현상을
재현했다. 10초 지연 시 정상 동작했고 클라이언트 정적 자산은 최신이었다.
`useHydrated`로 수납 등록 및 영수증 취소 컨트롤을 이벤트 연결 완료 전까지
비활성화했다. 최종 테스트에 고정 대기는 남기지 않았다.
서버 HTML의 비활성 상태와 확인·닫기·실패·성공 회귀 검사를 추가했다.
로그인 세션의 모달 열기·닫기 검사는 모든 변경 요청을 차단하며 실제 void는 실행하지 않는다.
