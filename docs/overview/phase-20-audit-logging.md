...
### 2026-09-05 관리자 변경 이력 화면 개발 재개

- 중단된 작업에는 API 조회 제한 회귀 테스트와 화면 테스트만 남아 있었다.
- `parseInt`가 `10x`를 허용하고 음수·중복 쿼리를 차단하지 않는 원인을 수정했다.
  `limit`은 1~100, `offset`은 0~1,000,000의 정수 문자열만 허용한다.
- `/admin/audit-logs` 화면은 `user:manage` 확인 후 서버 인증 요청으로 조회한다.
  대상 종류·ID·작업·수행자 필터, 변경 metadata와 이전 완료 기록, 서울 시간 표시를 제공한다.
  페이지마다 51건을 요청해 50건을 표시하고 다음 페이지 유무를 판단한다. 전체 건수는 집계하지 않는다.
  관리자 메뉴·페이지 검색에도 동일한 권한 조건을 적용한다.
- 확인된 검증: API HTTP 13개, 컨트롤러 6개, 웹 전체 200개 테스트 통과.
  추가 서버 조회 5개(필터 인코딩·인증 요청·오류 보존·세션 실패)와 관련 화면·메뉴 총 12개 검사 통과.
  API 빌드, 웹 `tsc --noEmit`, 변경 파일 ESLint도 통과했다. 실제 로그인 브라우저 검수는 미완료다.
- 감사 원본 데이터와 업무 변경 경로는 수정하지 않았다. 과거 외부 접근 여부 조사는 별도 미완료다.
- Cloudflare 실제 로그인 검수: 현재 계정의 감사 화면 접근 거부와 사이드바·검색 미노출 검사 통과.
  관리자 필터·잘못된 페이지 복구 검사는 `user:manage` 권한 부족으로 실패했다.
  `web/e2e/audit-history.spec.ts`를 관리자 테스트 세션으로 재실행해야 한다.
  별도 리뷰 에이전트는 사용량 제한으로 실행하지 못했으므로 독립 리뷰 완료로 기록하지 않는다.
