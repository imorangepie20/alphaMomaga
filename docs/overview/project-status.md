...
44. `web` CI에 안정화된 Properties/운영 페이지 브라우저 테스트 편입
45. `web` 로그인 UI와 외부 OAuth2/OIDC Bearer 인증 경계의 현재 상태 및 구현 조건 문서화
46. `web` Properties 페이지와 실제 API 응답을 비교하는 브라우저 통합 테스트 추가
47. `web` 5개 운영 페이지와 실제 API 응답을 비교하는 브라우저 통합 테스트 추가
48. Web CI에서 API를 기동하고 실제 API 응답 비교 브라우저 테스트 실행
